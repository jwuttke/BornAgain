...
It is an essential ingredient for the scattering theory developed below.
The vacuum Green function~$\TGo$ obyes the differential equation
\begin{equation}
  \TDo \TGo(\r,\r') = \v1 \delta(\r-\r'),
\end{equation}
plus a boundary condition to ensure that the solution is an \E{outgoing} wave.
In the scalar case, this solution is
...
\begin{equation}\label{EGo2}
  \TGo(\r,\r') = \v1 g(|\r-\r'|);
\end{equation}
for X-rays \cite{DiWa84}
\begin{equation}\label{EGo3}
  \TGo(\r,\r') = (\v1 + K^{-2}\Nabla\otimes\Nabla) g(|\r-\r'|).
\end{equation}
All these functions obye source-sink reciprocity,
\begin{equation}\label{EGoReci}
  \TGo(\r,\r')=\TGo(\r',\r),
\end{equation}
Ultimately, we will be interested in the radiation intensity in a detector
that is located far away from the scattering target.
Therefore we need the far-field limit of the Green function,
...
\begin{equation}\label{EGoFar2}
  \TGo^\infty(\r,\r') = (\v1 - \v{\hat r}\otimes\v{\hat r}) G_0(\r,\r')
\end{equation}
...
and reflection
\index{Reflection}%
can distort an incident plane wave
quite substantially.
This motivates the
\index{Distorted-wave Born approximation}%
...
The vacuum wave equation that is solved by plane-wave states \cref{EvacWave}
