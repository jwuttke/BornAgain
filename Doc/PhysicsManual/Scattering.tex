...
\begin{equation}
   \hat V_\text{magn} = -\v{\hat \mu}\v{B} = + \mu\PauliVec\v{B},
\end{equation}
where a hat indicates operators on spin states, represented by $2\times2$ matrices.
...
\begin{equation}\label{EGrspace}
  ...
  = (2\pi)^{-3} \sum_\alpha \int\!\d^3k\int\!\d^3r''\,
        \v\Phi_\ka(\r)\Cdot \hu_\alpha \e^{-i\k\r''} \TGo(\r'',\r').
\end{equation}
...
\begin{equation}\label{EGo3}
  \TGo(\r,\r') = (\v1 -K^{-2}\Nabla\Cdot\Nabla) G_0(\r,\r').
\end{equation}
...
The $\Nabla\Cdot\Nabla$ term in \cref{EGo3}
...
\begin{equation}\label{EGresult}
  ...
  = \sum_\alpha \v\Phi_{\k_\sf\alpha}(\r)\Cdot \hu_\alpha g(r').
\end{equation}
