...
%The chapter finishes with a qualitative discussion of coherence lengths in \Cref{Scoherlen}.
...
In DWBA, we need to distinguish the \E{exciting} and the \E{incident} wave;
...
In \cref{SvacGreen}, we computed the far-field vacuum Green function $\TGo^\infty$
...
As we have seen, the simple form~\cref{Exsection2} of the DWBA cross section
depends on the symmetry~\cref{Erecip} of the Green function.
This is one of several forms of \E{reciprocity} in wave propagation
\cite{Pot04,DeFu12a}.
It is not a fundamental physical law,
but must be proven case by case for each specific wave equation.
This proof will be untertaken here for the different wave operators $\TD$~\cref{EDo}
and for scalar and vectorial distortion fields~$\TL$.
Readers not interested in mathematical details may skip the remainder of this section.

Proofs are greatly facilitated if energy conservation can be used.
This shortcut shall not be taken here
because BornAgain also supports scattering from absorbing media.

For scalar neutron waves,
we write the reciprocity theorem as
\begin{equation}\label{Erecip2}
  G(\rS,\rD) = G(\rD,\rS)
\end{equation}
with source location~$\rS$ and detection point~$\rD$.
\nomenclature[2r041 2d100]{$\rD$}{Position of detector}%
\nomenclature[2r041 2s100]{$\rD$}{Position of source, locus of scattering}%
We introduce the auxiliary vector field%
\begin{equation}\label{EdefX}
...
\end{equation}
We apply $\Nabla$ to the right-hand side of~\cref{EdefX} and obtain
\begin{equation}
  \Nabla\v{X}(\r,\rS,\rD)
  = G(\r,\rD)\Nabla^2 G(\r,\rS) - G(\r,\rS)\Nabla^2 G(\r,\rD).
\end{equation}
We use the wave equation
\begin{equation}
  [\Nabla^2+K^2-\Lambda(\r)]G(\r,\r') = \delta(\r-\r'),
\end{equation}
and carry out the integral to obtain
...
\begin{equation}
  ...
  = 0.
\end{equation}
Comparison with \cref{EIBD} completes the proof of~\cref{Erecip2}.
